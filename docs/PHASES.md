...
- `mypy` (strict, 132 files) — clean
- `pytest` — **229 passed**
- `python -m apex --check` — boots healthy (3 plugins, 4 modules), exits 0
- Real-data validation: structure family computed over 99 confirmed live
  Toobit bars per symbol (1,558 features each); readings coherent with the
  observed BTC rally (bullish trend, BOS/CHoCH transitions, premium range)
...
1. Liquidity family: equal-high/low confidence pools with decay, sweeps,
   inducement, internal/external pivots (AICE lines ~1203-1245).
2. Order blocks + FVG family (AICE OB/FVG types and lifecycle).
3. Volume & normalization family (percentile ranks, z-scores, volume
   confirmation), then SMT divergence (multi-symbol context).
4. Then Phase 5: the Probability Platform consuming stored feature vectors.
