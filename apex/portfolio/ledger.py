...
entry bar, same-bar collisions resolved conservatively. Fills are
ideal (exact stop/target prices, no fee or slippage model) until the
execution engine lands (Phase 10) - documented deferral.
...
def close_lot(
    lot: OpenLot, bar: Bar, *, conservative: bool, currency: str
    ...
    on, to the target otherwise. Returns the closed position version
    and its trade record, or ``None`` when the lot stays open.
    """
    ...
    pnl_amount = (exit_price - lot.entry) * quantity * sign
